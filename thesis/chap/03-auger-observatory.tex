...
PMT exceeding a $2.5\,I_\text{Peak}^\text{est.}$ is used to determine a calibration trigger rate. If for some reason not all three WCD PMTs are functional, the 
thresholds are altered according to \autoref{tab:calib-trigger-thresholds}. What follows is an iterative procedure to approximate \Ipeak (\Qpeak) for each PMT 
separately:

\begin{enumerate}
	\item Calculate the trigger rate of the calibration trigger over a time $t_\text{cal.} = \SI{5}{\second}$.
	\item If the trigger rate is not within a rage of $\SI{70\pm2}{\hertz}$ alter $I_\text{Peak}^\text{est.}$ ($Q_\text{Peak}^\text{est.}$) by $\mp\SI{1}{\ADC}$.
	\TODO
\end{enumerate}

\begin{table}
	\begin{center}
	\caption{Calibration trigger thresholds for every station}
...
What follows is an iterative procedure to approximate 
\Ipeak (\Qpeak):Next, the 
rate of events satisfying a calibration trigger is calculated across a calibration time $t_\text{cal}$.

\begin{itemize}
	\item 
\end{itemize}

\section{Trigger procedure}
\label{sec:trigger-procedure}
