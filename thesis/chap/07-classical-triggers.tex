...
raise a T2. The trigger probability obtained this way must however not be confused with the lateral trigger probabilty discussed above. It is the conditional 
...
is shown in \autoref{fig:LPP-fits}. The best fit parameters are listed in \autoref{app:LPP-fit-parameters}.
...
The comparison between the LTP gathered via Bayesian folding is compared to the \Offline approach in \autoref{sec:offline-ltp}. This is done for every classical
trigger individually. It is found that no remarkable difference between both results exists. As a consequence, this method of evaluating the LTP allows for an 
easy comparison of neural network triggers to the algorithms discussed in the following. 
...
this to the nominal T3 trigger rate at CDAS level ($\approx\SI{0.03}{\hertz}$ \cite{abraham2010trigger}), it becomes obvious that a lot of background events pass 
this threshold. Consequently, the trigger has a very high false discovery rate 
$\text{FDR}\approx\frac{\SI{20}{\hertz} - \SI{0.03}{\hertz}}{\SI{20}{\hertz} + \SI{0.03}{\hertz}}=0.997$.
