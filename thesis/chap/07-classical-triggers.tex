...
showcasing this result is offered in \autoref{fig:fitfunction-comparison}

\begin{figure}
	\centering
	\includegraphics[width=1\textwidth]{./plots/LTP_bias_check.png}
	\caption{The residuals from comparing $\epsilon^\text{True}$ to $\text{LTP}(r)$ vanish at large (small) $r$, where the lateral trigger probability is close to
	zero (one). No systematic bias is observed in the transitional region around $R^*$. Large outliers are caused by the dataset being limited in site at low 
	energies (marked with an x)}
