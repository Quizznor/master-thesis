...
(\SI{120}{\mega\hertz}) than that of UB electronics (\SI{40}{\mega\hertz}). The effect this has on measured data is visualized in \autoref{fig:uub-ub-comparison}. 
Whereas a new FADC bin is measured every \SI{25}{\nano\second} in a UB station, the triggers would receive a new input every $\approx\SI{8.3}{\nano\second}$ in a 
UUB setting. This drastically reduces the efficiency (in particular for ToT-like triggers) of the above discussed algorithms.

One way to circumvent this problem is adjusting the hyperparameters 
