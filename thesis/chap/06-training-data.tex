...
\begin{figure}
	...
	\caption{(\textit{Left}) A simulated signal as it would appear to UUB electronics. The ionizing particles originating in the extensive air shower hit the tank 
		around bin 660 ($\approx\SI{5.5}{\micro\second}$). (\textit{Right}) The same signal but filtered and downsampled to emulate UB electronics.}
