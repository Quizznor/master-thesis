...
floored to mimic the FPGA digital counting, and divided by the appropriate scaling factor \Ipeak. If traces need to be UB compatible (compare 
\autoref{ssec:filtering-and-downsampling}), the trace must be filtered and downsampled in an intermediate step, the scaling factor becomes 
$I_\text{VEM, compat.} = \SI{163.235}{\ADC}$ \cite{OfflineSource}. If this isn't the case, the appropriate factor is $I_\text{VEM, Off.}$.
...
$2^{11} = 2048$, but also drastically reduces the efficiency (in particular for ToT-like triggers) of the above discussed algorithms. Whereas a new  bin is 
