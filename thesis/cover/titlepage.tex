...
\newcommand{\imagetopleft}{(29mm, -125mm)}
...
\begin{titlepage}
	...
	\begin{tikzpicture}[remember picture,overlay,every node/.style={inner sep=0,outer sep=0}]
		...
		\node[anchor=north] at ($(pagetopcenter) + (0mm,-\tpborder -100mm)$)
		{
		...
		% 	($(current page.north west) + (31mm, -120mm)$)
		% 	--
		% 	($(current page.north west) + (31mm, -100mm)$)
		% 	arc (180:90:2mm)
		% 	($(current page.north west) + (33mm, -98mm)$)
		% 	--
		% 	($(current page.north west) + (53mm, -98mm)$);

		% 	\draw[line width = 0.8mm, color=gray]
		% 	($(current page.north west) + (148mm, -183mm)$)
		% 	--
		% 	($(current page.north west) + (178mm, -183mm)$)
		% 	arc (270:360:2mm)
		% 	($(current page.north west) + (180mm, -181mm)$)
		% 	--
		% 	($(current page.north west) + (180mm, -161mm)$);

		% \node[anchor=north west] at ($(current page.north west) + (33mm, -100mm)$){\includegraphics[width=14.5cm]{cover/cover.png}};

		\node[anchor=north] at ($(pagetopcenter) + (0mm,-\tpborder - 140mm)$)
		{
			\begin{minipage}{160mm}
				\centering
				\tpfont
		...
		\node[anchor=south] at ($(pagebotcenter) + (0mm,\tpbotborder + 20mm)$)
		{
			\begin{minipage}{160mm}
				...
				\begin{tabular}[ht]{l c l}
					...
					Supervisors: & \hfill & \thesissupervisors\\
				\end{tabular}
