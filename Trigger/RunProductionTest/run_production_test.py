...
class HardwareClassifier():
    ...
    def Th1(self, signal) -> bool : 
        return self.Th(signal, 1.75)

    def Th2(self, signal) -> bool : 
        return self.Th(signal, 3.20)
...
# for increment in increments:
    ...
    n_all, n_th, n_tot, n_totd, n_mops = 0, 0, 0, 0, 0
    ...
    for j, trace in enumerate(Buffer._these_traces):
        ...
        th2_flag = Th2(downsampled_trace)
        tot_flag, totd_flag, mops_flag = False, False, False
        ...
        for (start, stop) in zip(window_start, window_stop):
            ...
            if not tot_flag: tot_flag = ToT(window)
            if not totd_flag: totd_flag = ToTd(window)
            if not mops_flag: mops_flag = MoPS(window)

            if np.all([tot_flag, totd_flag, mops_flag]): break

        n_all += max([th2_flag, tot_flag, totd_flag, mops_flag])
        n_th += th2_flag
        n_tot += tot_flag
        n_totd += totd_flag
        n_mops += mops_flag

        # print(n_all, n_th, n_tot, n_totd, n_mops, end = "\r")

    perc_str = str(int(percentage * 100)).replace('-','')

    with open(f"/cr/users/filip/Trigger/RunProductionTest/trigger_output_componentwise/{station}/{station}_all_triggers_componentwise_{pm}{perc_str}.csv", "a") as f:
        f.write(f"{Buffer.random_file} {n_traces} {duration} {n_all} {n_th} {n_tot} {n_totd} {n_mops}\n")
