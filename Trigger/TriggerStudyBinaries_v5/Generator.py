...
class Generator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index : int, full_trace : bool = False) -> tuple[np.ndarray] :
        ...
        # for trace in traces:
        #     print(trace.shape)
        #     if trace.shape != (3,120): 
        #         print(len(traces), len(self.__sliding_window__(VEMTrace)))
        #         raise StopIteration
    ...
    def check_sliding_window(self, batch : int) -> None :
        ...
        for i in self.__sliding_window__(traces[0]):
            ...
            window, n_sig = traces[0].get_trace_window((i, i + self.window_length))
        ...
        if full_traces:
            for batch in range(n_traces):
                ...
                for trace in traces:
                    ...
                    #     window, n_sig = trace.get_trace_window((i, f))
                    #     integral_signal = trace.integrate(window)
                    #     if self.ignore_low_VEM: n_sig = 0 if integral_signal < self.ignore_low_VEM else n_sig

                    #     if n_sig: has_label.append(integral_signal)
                    #     else: has_no_label.append(integral_signal)
