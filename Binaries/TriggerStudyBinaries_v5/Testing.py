            ...
            for VEMTrace in traces:
                ...
                for i in Dataset.__sliding_window__(VEMTrace):
                    ...
                    if n_sig:
                        ...
                        save_string = f"{signal:.3f} {n_sig} {VEMTrace.Energy:.3e} {VEMTrace.SPDistance:.0f} {VEMTrace.Zenith:.3f}"
...
def ROC(save_path : dict, **kwargs) -> None :
    ...
    print(save_path.ljust(60), f"{f'{TPs}'.ljust(7)} {f'{FPs}'.ljust(7)} {f'{TNs}'.ljust(7)} {f'{FNs}'.ljust(7)} -> Acc = {ACC:.2f}%, TPR = {TPR:.4f}%")
    ...
    plt.plot(ROC_x, ROC_y, c = kwargs.get("c", "steelblue"), label = " ".join(save_path.split("_")[:5]), ls = kwargs.get("ls", "solid"), lw = 2)
