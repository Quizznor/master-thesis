...
void DoLtpCalculation(fs::path pathToAdst)
{
  // const auto csvTraceFile = pathToAdst.parent_path()/ pathToAdst.filename().replace_extension("csv"); // for testing
  const auto energyRange = pathToAdst.string().substr(39, 7);
  const std::string csvLtpFileMisses = "/cr/tempdata01/filip/QGSJET-II/LTP/lateral_trigger_probability_" + energyRange + "/misses.csv";
  const std::string csvLtpFileHits = "/cr/tempdata01/filip/QGSJET-II/LTP/lateral_trigger_probability_" + energyRange + "/hits.csv";
