...
root_path = "/cr/tempdata01/filip/QGSJET-II/QUENTIN/lge18.0-18.5"
...
for k, file in enumerate(steps):
    ...
    data = np.loadtxt(root_path + "/" + file, max_rows = 66)

    #   saveFile << (i + 1) * 100 << " " << all_hits[i] << " " << misses[i] << " " << th1_hits[i] << " " << th2_hits[i] << " " << tot_hits[i] << " " << mops_hits[i] << "\n";
    _, log_e, theta, _, _, _ = data[0]
    _, hits, misses, th2, tot, totd = data[1:, 0], data[1:, 1], data[1:, 2], data[1:, 3], data[1:, 4], data[1:, 5]
...
for e_bin in range(1, len(energy_bins)):
    ...
    for t_bin in range(1, len(theta_bins)):
        ...
        save_file = f"/cr/tempdata01/filip/QGSJET-II/QUENTIN/BINNED/{energy_bins[e_bin - 1]}_{energy_bins[e_bin]}__{int(theta_bins[t_bin - 1])}_{int(theta_bins[t_bin])}.csv"
        save_hits, save_misses = hits_sorted[e_bin - 1][t_bin - 1], miss_sorted[e_bin - 1][t_bin - 1]
        save_th2 = th2_sorted[e_bin - 1][t_bin - 1]
        save_tot, save_totd = tot_sorted[e_bin - 1][t_bin - 1], totd_sorted[e_bin - 1][t_bin - 1]
        save_matrix = np.dstack([sp_distances, save_hits, save_misses, save_th2, save_tot, save_totd])[0]
