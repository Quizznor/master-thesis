  ...
  for (unsigned int PMT = 1; PMT < 4; PMT++)
  {
    ...
    for (unsigned int i = 0; i < recEventFile.GetNEvents(); ++i) 
    {
      ...
      for (const auto& recStation : sdEvent.GetStationVector()) 
      {
        ...
        auto total_trace = VectorWrapper(trace_container);

        // calculate component trace
        // 1 = photons, 2 = electrons, 3 = muons
        for (int component = ePhotonTrace; component <= eMuonTrace; component++)
        {
          ...
          auto calibrated_trace = VectorWrapper( component_trace.GetVEMComponent() );

          if (calibrated_trace.values.size() != 0)
          {
        ...
        auto result = total_trace.get_trace();
