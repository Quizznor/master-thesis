...
def pmap(function, arguments, numprocesses=None, chunksize=None, async=False):
  ...
  proc = [mp.Process(target=FuncWorker, args=(function, q_in, q_out)) for _ in range(numprocesses)]
...
def cached(keepOpen=False, lockCacheFile=False, trackCode=True):
  ...
  if isinstance(keepOpen, FunctionType):
    ...
    prog = os.path.splitext(os.path.basename(prog))[0] if os.path.exists(prog) else "python"
...
def cached_at(cacheFileName, keepOpen=False, lockCacheFile=False, trackCode=True):
  ...
  def decorator(function):
    ...
    def decorated_function(*args, **kwargs):
      ...
      # if the user decides to recall the function and manually recall the cache, it will still work
      ...
      if key in d:
        assert "cache" in d[key], "Your cache might be outdated. Try to delete and create it again!"
        if trackCode:
