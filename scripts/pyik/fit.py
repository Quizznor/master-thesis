...
class Minimizer(object):
  ...
  def __call__(self, function, starts):
    ...
    for ipar in xrange(nExt):
      s = starts[ipar]
      l = -np.inf if self.lower_bounds is None else self.lower_bounds[ipar]
      u = np.inf if self.upper_bounds is None else self.upper_bounds[ipar]
      if s < l or u < s:
        raise ValueError("Par %i: starting value %g not in range [%g,%g]" % (ipar, s, l, u))
      if l == u:
        fix.append(ipar)
      else:
        iStarts.append(s)
...
class ChiSquareFunction(object):
  ...
  def __init__(self, model, xs, ys, yerrs=None, xerrs=None):
    ...
    if ys.ndim != 1:
      ...
      self.xerrs = None if xerrs is None else np.array([t[mask].flatten() for t in xerrs])
    else:
      self.xs = xs[mask]
  ...
  def __call__(self, par, grad=None):
    ...
    parmodel = lambda x: self.model(x, par)

    yerrs2 = self.yerrs2
    if self.xerrs is not None:
      eys = np.frompyfunc(lambda x, xerr: derivative(parmodel, x) * xerr, 2, 1)(self.xs, self.xerrs)
      # yerrs2 += eys * eys
    ...
    try:
      ...
      raise TypeError("model is not vectorized, see numpy.frompyfunc and numpy.vectorize")
