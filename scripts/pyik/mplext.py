...
def ViolinPlot(x, y, bins=10, range=None, offsetX=0, offsetY=0,
               ...
               extend=3, outliers=True, axes=None, **kwargs):
  """
  ...
  draw: string (default: "amv")
    What to draw: a (mean), m (median), v (violins)
  ...
  for i, ybin in enumerate(ybins):
    ...
    if len(ybinh) > 1:
      ...
      if "m" in draw:
        ...
        mask = (y > meds[i] - (meds[i] - qs[1]) / np.sqrt(ns[i])) & (y < meds[i] + (qs[2] - meds[i]) / np.sqrt(ns[i]))
      ...
      if outliers:
        ...
        plt.plot(xouts + outliers, youts, marker=".", ls="None", ms=2, color=color, zorder=1)

  # Mean profile
  if "a" in draw:
    merrbar = plt.errorbar(xcens + offsetX, means, stds / np.sqrt(ns), marker=marker, ls="None",
                           elinewidth=1, mew=2, mfc="white", mec=color, color=color, ms=6,
  ...
  if "c" in draw:
    for n, x in zip(ns, xcens + offsetX):
      plt.annotate(str(n.astype(int)), xy=(x, y.min() + offsetY), xycoords=('data', 'data'),
                   xytext=(0, 0), textcoords='offset points', va='top', ha='center', color=color, size=15)
