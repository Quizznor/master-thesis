...
def profile(x, y, bins=10, range=None, sigma_cut=None):
  ...
  ystd : array of dtype float
    ...
    uncertainty of ymean, calculate: yunc = ystd/numpy.sqrt(n).
  ...
  for i in xrange(n):
    for j in xrange(i, n):
      ...
      if i == j:
        ...
        hesse[i, i] = (f(xmm) + f(xpp) - f(xp) - f(xm)) / (3.0 * steps[i] * steps[i])
...
def propagate_covariance(f, x, cov):
  ...
  for icol in xrange(ncol):
    dx[icol] = np.sqrt(cov[icol][icol]) * 1e-3 if cov[icol][icol] else 1.0
...
def quantiles(ds, qs, weights=None):
  ...
  if weights is None:
    ...
    assert len(ds) == len(weights), "Data and weights arrays need to have equal length!"
    assert np.all((qs >= 0) & (qs <= 1)), "Quantiles need to be within 0 and 1!"
...
class ConvexHull:
  ...
  Authors
  -------
  Ariel Bridgeman <ariel.bridgeman@kit.edu>, prior work done by Hans Dembinski <hans.dembinski@kit.edu>
  """
...
def poisson_uncertainty(x):
  ...
  Parameters
  ----------
  x: array-like or scingle integer
...
def LOOCV(function, xs, ys, estimates, xerrs=None, yerrs=None):
  """
  Perform a Leave-one-out cross-validation of the prediction power of function assuming normally distributed values.
  ...
  for i in range(len(xs)):
    ...
    pars_i = ChiSquareFunction(function, np.delete(xs, i), np.delete(ys, i),
                               xerrs=DelIfNotNone(xerrs, i), yerrs=DelIfNotNone(yerrs, i)).Minimize(starts=estimates)[0]
...
class FeldmanCousins(object):
  ...
  def EstimateOptimumParameter(self, x):
    ...
    Internal function.
    """
    minfct = lambda pars: -self._pdf(x, pars[0])
  ...
  def GetVariableLimits(self, mu):
    ...
    if self._pdftype == "poisson":
      ...
      mubest = self.Boundarize(np.vectorize(self.EstimateOptimumParameter)(xcont))
...
class multivariate_gaussian_evaluator(object):
  ...
  Authors
  -------
  Ariel Bridgeman <ariel.bridgeman@kit.edu>, with credit for is_in_coverage to Hans Dembinski
  """
  ...
  def is_in_coverage(self):
    ...
    for p in xrange(len(points)):
      ...
      isin[p] = int(np.sum(x * x) <= chi2(len(self.mean.copy())).ppf(self.coverage))
