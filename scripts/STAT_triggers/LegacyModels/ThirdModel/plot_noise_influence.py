...
for file in os.listdir("/cr/data01/filip/second_model/noise_studies"):
    ...
    # plt.errorbar(std * 61.75, accuracy, marker = marker, c = color, ls = None, yerr = 1/np.sqrt(confusion_matrix.sum()))
...
for file in os.listdir("/cr/data01/filip/third_model/noise_studies"):
    ...
    plt.scatter(std * 61.75, accuracy, marker = "s", c = "green")
    # plt.errorbar(std * 61.75, accuracy, marker = "s", c = "green", ls = None, yerr = 1/np.sqrt(confusion_matrix.sum()))
...
plt.scatter([],[], marker = "s", c = "g", label = "Sequential + pooling")
