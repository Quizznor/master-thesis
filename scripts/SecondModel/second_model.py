...
class EventGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, dataset, *args) -> None :
        ...
        unique_events = np.unique([event[:13] for event in os.listdir("/cr/data01/filip/" + dataset)])
    ...
    def __getitem__(self, index) -> tuple :
        ...
        for station in glob.glob("/cr/data01/filip/" + self.__path_to_dataset_folder + self.__files[index] + "*.csv"):
...
if __name__ == "__main__":
    ...
    EventClassifier.save("/cr/data01/filip/" + "second_simulation/tensorflow/model/")