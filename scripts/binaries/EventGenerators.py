    ...
    def __new__(self, dataset : str, **kwargs) -> typing.Any :
        ...
        class Generator(tf.keras.utils.Sequence):
            ...
            def __init__(self, dataset, *args):
                ...
                unique_events = os.listdir("/cr/data01/filip/" + dataset)
                start = 0 if not args[0] else 0 if split >= 0 else int(args[1] * len(unique_events))
                stop =  -1 if not args[0] else int(args[1] * len(unique_events)) if split > 0 else -1
        ...
        split     = set_kwarg(kwargs, 'split', 0.8) if train else 1             # default to splitting 80% / 20% (Train / Validation)
        ...
        TestingSet = Generator(dataset, train, split, shuffle, bkg_size, bkg_std, bkg_mean, imbalance, pooling)
