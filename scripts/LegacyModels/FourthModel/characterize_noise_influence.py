...
EventClassifier = Classifier("/cr/data01/filip/fourth_model/model_2")
...
for step, std in enumerate(stds, 1):
    ...
    PredictionDataset, _ = DataSetGenerator("02_simulation/signal/", train = True, baseline_std = std, split = 1.0)
    ...
    np.savetxt(f"/cr/data01/filip/fourth_model/noise_studies_gen_2/{std}_network.txt", network_confusion_matrix)