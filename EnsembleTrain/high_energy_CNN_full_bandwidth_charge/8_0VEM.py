...
try:
    ThisNN = NNClassifier(f"ENSEMBLES/120_TwoLayer_FullBandwidth_HighEnergies_8_0VEM/ensemble_{ensemble_no:02}")
except FileNotFoundError:
    MockNN = NNClassifier("ENSEMBLES/120_TwoLayer_FullBandwidth_HighEnergies_NoCuts/ensemble_01", supress_print = True)
    Events = EventGenerator([], ignore_low_vem = 8.0)          # cuts will go here
    Events[0].files += MockNN.get_files("training")
    Events[1].files += MockNN.get_files("validation")

    ThisNN = NNClassifier(f"ENSEMBLES/120_TwoLayer_FullBandwidth_HighEnergies_8_0VEM/ensemble_{ensemble_no:02}", "two_layer_conv2d")
