...
# Events = EventGenerator([], ignore_low_vem = 1.0)
# Events[0].files += MockNN.get_files("training")
# Events[1].files += MockNN.get_files("validation")

# ThisNN = NNClassifier(f"ENSEMBLES/120_TwoLayer_FullBandwidth_AllEnergies_1_0VEM/ensemble_{ensemble_no}", "two_layer_conv2d")
# ThisNN.train(Events, 10)

ThisNN = NNClassifier(f"ENSEMBLES/120_TwoLayer_FullBandwidth_AllEnergies_1_0VEM/ensemble_{ensemble_no}")
